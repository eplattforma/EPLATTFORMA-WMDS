...
from routes_dropbox import dropbox_bp
...
def sync_ps365_customers():
    ...
    try:
        ...
        if result.get('success'):
            flash(f'✅ Customer sync completed! {result["updated_count"]} customers synced from PS365.', 'success')
        else:
            flash(f'❌ Customer sync failed: {result.get("error", "Unknown error")}', 'danger')
    except Exception as e:
        app.logger.error(f"Customer sync error: {str(e)}")
        flash(f'❌ Customer sync error: {str(e)}', 'danger')
...
PERMISSION_EDITOR_GROUPS = [
    ...
    ("Cooler", [
        ...
        "cooler.lock_sequencing", "cooler.print_labels",
    ]),
    ("Routes & Sync", [
...
PERMISSION_DESCRIPTIONS = {
    ...
    # Cooler
    "cooler.pick":            "Pick items on cooler (SENSITIVE) routes — the cooler picking queue and item pick/skip/exception actions.",
    "cooler.manage_boxes":    "Plan, create, assign items to, close, reopen, and cancel cooler boxes; confirm box plans.",
    "cooler.lock_sequencing": "Confirm a cooler route and lock its delivery sequence so items can be sorted into boxes in stop order.",
    "cooler.print_labels":    "Print cooler route status sheets, box labels, and manifests.",
    # Routes & Sync
    ...
    },
    {
        ...
        'flags': [
            ...
            {'key': 'allow_legacy_session_picking_fallback', 'label': 'Legacy Session Fallback',
             ...
             'risk': 'yellow', 'kind': 'bool'},
            {'key': 'route_batch_mode_enabled', 'label': 'Route Batch Mode',
             'description': 'When ON, normal items auto-accumulate into ROUTE-BATCH-<id> when invoices join a route. Lock gate required before picking.',
             'risk': 'yellow', 'kind': 'bool'},
...
def picker_dashboard():
    ...
    for inv in invoices_raw:
        ...
        try:
            # Create a subtransaction/savepoint to handle specific query failures
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if confirm:
                ...
                if batch_record:
                    batch_record.picked_qty = picked_qty
                    current_app.logger.info(f"🔄 SYNC FIX: Updated batch record for {current_item.item_code} to {picked_qty} (individual pick)")
                    db.session.add(batch_record)
