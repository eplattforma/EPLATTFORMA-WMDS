...
The following matrix is **automatically generated** by `tests/test_phase3_closeout_matrix.py` (parametrised, 35 cells). Each cell drives the Flask test client against a representative route for that key, with a freshly-seeded user of that role and `permissions_enforcement_enabled = 'true'` flipped inside a transactional fixture (no live flag mutation). The captured `PHASE3_CLOSEOUT_MATRIX.txt` snapshot is regenerated on every run and committed alongside the test.
