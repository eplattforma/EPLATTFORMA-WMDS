...
The following matrix is **generated on demand** by `tests/test_phase3_closeout_matrix.py` (parametrised, 35 cells). Each cell drives the Flask test client against a representative route for that key, with a freshly-seeded user of that role and `permissions_enforcement_enabled = 'true'` set via the `enforcement_on` fixture. The captured `PHASE3_CLOSEOUT_MATRIX.txt` snapshot is **only** rewritten when the test is run with `PHASE3_REGEN_MATRIX=1` in the environment (`PHASE3_REGEN_MATRIX=1 pytest -q tests/test_phase3_closeout_matrix.py`); normal `pytest` invocations assert behaviour without churning the tracked closeout artefact. The snapshot in this commit was regenerated using that workflow on 2026-05-02.
...
**Result:** ✅ **42 / 42 PASSED** (33 original + 9 fix-up regressions covering rounds 1 & 2, ~9s, in-memory SQLite)
**Phase 4 regression:** ✅ `tests/test_phase4_batch_picking.py` 30/30 PASSED.
...
| FIX-05 | `update_phase5_cooler_picking_schema.py` used Postgres-only DDL (`BIGSERIAL`, `TIMESTAMP WITH TIME ZONE`, `ADD COLUMN IF NOT EXISTS`); migration crashed under SQLite test dialect and on first cold app boot | Rewrote dialect-aware: Postgres path keeps `BIGSERIAL`/TS-TZ/FK clauses + native `ADD COLUMN IF NOT EXISTS` (race-safe under multi-worker boot, ASSUMPTION-028); SQLite path emits `INTEGER PRIMARY KEY AUTOINCREMENT`/`TIMESTAMP`, `ADD COLUMN` guarded by `inspect()` reflection with duplicate-column swallow as defence-in-depth | `test_p5_fix_05_migration_runs_on_sqlite_dialect` (re-run to prove idempotency) |
| FIX-06 | Round-2 architect rejection: cooler blueprint queries (`route_list`, `route_picking`, `route_manifest`) filtered on `Invoice.routing` (a free-text label) and `Invoice.upload_date`, while `cooler_boxes.route_id` and the rest of the dispatch system key on `Invoice.route_id` (FK to `shipments.id`). When a planner relabels a route the queries returned the wrong picker work-list and mis-attributed boxes | All three queries now filter on `i.route_id = :route_id` (int cast at the route boundary) and join `shipments` for `s.delivery_date` (ASSUMPTION-029). The route manifest short-circuits and filters directly on `cooler_boxes.route_id` since that column is itself the FK | `test_p5_fix_06_cooler_blueprint_keys_on_route_id_not_routing` |
