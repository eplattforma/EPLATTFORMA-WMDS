...
class BatchPickingSession(db.Model, SoftDeleteMixin):
    ...
    cooler_pack_mode = db.Column(db.String(20), default='location_order')
    cooler_box_type_id = db.Column(db.Integer, nullable=True)
    ...
    def get_grouped_items(self, include_picked=False):
        ...
        if self.picking_mode == 'Cooler':
            # Cooler-route batches: all items are locked by locked_by_batch_id == self.id
            # and have pick_zone_type='cooler' in the queue. The normal zone filter
            # (InvoiceItem.zone vs "SENSITIVE") does not apply — InvoiceItem.zone stores
            # the warehouse aisle zone, not the DW classification. We must NOT exclude
            # cooler_keys here because every item in this batch IS a cooler item.
            cooler_filter_conds = [InvoiceItem.locked_by_batch_id == self.id]
            if not include_picked:
                cooler_filter_conds.extend([
                    InvoiceItem.is_picked.is_(False),
                    InvoiceItem.pick_status.in_(['not_picked', 'reset', 'skipped_pending']),
                ])
...
class DwItem(db.Model):
    ...
    cost_price_updated_at = db.Column(db.DateTime, nullable=True)
...
class Ps365Oos777Daily(db.Model):
    __tablename__ = "ps365_oos_777_daily"
    ...
    supplier_name = db.Column(db.String(255), nullable=True)
    barcode = db.Column(db.String(100), nullable=True)
    stock = db.Column(db.Numeric(18, 4), nullable=True, default=0)
    stock_reserved = db.Column(db.Numeric(18, 4), nullable=True, default=0)
    available_qty = db.Column(db.Numeric(18, 4), nullable=True, default=0)
    detected_at = db.Column(UTCDateTime(), nullable=False, default=get_utc_now)
    source_run_id = db.Column(db.Integer, db.ForeignKey("ps365_stock_777_runs.id"), nullable=True)

    __table_args__ = (
        db.Index("ix_oos_777_date_item", "snapshot_date", "item_code_365", unique=True),
    )

    def __repr__(self):
        return f"<Ps365Oos777Daily {self.snapshot_date} {self.item_code_365}>"
...
class ReplenishmentSupplier(db.Model):
    ...
    email_cc = db.Column(db.String(500), nullable=True)
    email_columns_json = db.Column(db.Text, nullable=True)
    is_active = db.Column(db.Boolean, nullable=False, default=True)
...
class SkuForecastProfile(db.Model):
    ...
    seasonality_source = db.Column(db.String(128), nullable=False, default='none')
    ...
    seed_source = db.Column(db.String(128), nullable=True)
    analogue_item_code = db.Column(db.String(64), nullable=True)
    analogue_level = db.Column(db.String(128), nullable=True)
    ...
    baseline_source = db.Column(db.String(128), nullable=True)
