...
- **Core Features**:
    ...
    - **Replenishment MVP**: Proposes case quantities to order per supplier based on current stock, weekday sales averages, and safety stock. First supplier: Corina Snacks Ltd (PS365 code `10000159`). Service modules in `services/replenishment_mvp/` (calendar, forecast, planner, ps365_client, repositories). Blueprint at `blueprints/replenishment_mvp.py` with URL prefix `/replenishment-mvp`. Models: `ReplenishmentSupplier`, `ReplenishmentItemSetting`, `ReplenishmentRun`, `ReplenishmentRunLine`. Schema migration: `update_replenishment_schema.py` (idempotent, uses IF NOT EXISTS / ADD COLUMN IF NOT EXISTS). Tuesday/Friday ordering cycles with receipt date calculations, pre-receipt demand deduction, cover period forecasting. Case qty resolution: `min_order_qty` → `replenishment_item_settings.case_qty_units` override → CASE_QTY_MISSING warning. Sales date field: `dw_invoice_header.invoice_date_utc0`. Warning system with priority ordering: CASE_QTY_MISSING > NEGATIVE_PROJECTED_STOCK > NO_RECENT_SALES > EXPIRY_SOON > HAS_ORDERED_STOCK > HAS_CURRENT_RESERVED > ZERO_ORDER. Run date validation: blocks future dates, warns for >7 days old. Debug diagnostics: forecast item counts and weekday sales coverage logged at INFO/DEBUG levels. `calc_json` stores `case_qty_source` and `sales_date_field` for audit trail.
