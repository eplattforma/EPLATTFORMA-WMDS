...
- **Core Features**:
    ...
    - **PS365 Stock 777 Sync**: Once-daily stock availability sync for Store 777 (Eshop). Service: `services/ps365_stock_777_service.py`. Tables: `ps365_stock_777_runs` (run log), `ps365_stock_snapshot_777_daily` (raw daily snapshots with unique index on date+item), `ps365_stock_777_current` (latest state per item). Uses `list_stock_items_store` GET endpoint, enriches with DwItem metadata (supplier, barcode, active status). Computes `available_qty = max(stock - stock_reserved, 0)` and derived flags (`is_available`, `is_oos`, `is_low_stock`). Scheduled daily at 5:30 AM via APScheduler. Downstream systems read from `ps365_stock_777_current`.
