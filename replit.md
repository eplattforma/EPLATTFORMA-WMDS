...
- **Core Features**:
    ...
    - **Replenishment MVP**: Proposes case quantities to order per supplier based on current stock, weekday sales averages, and safety stock. First supplier: Corina Snacks Ltd. Service modules in `services/replenishment_mvp/` (calendar, forecast, planner, ps365_client, repositories). Blueprint at `blueprints/replenishment_mvp.py` with URL prefix `/replenishment-mvp`. Models: `ReplenishmentSupplier`, `ReplenishmentItemSetting`, `ReplenishmentRun`, `ReplenishmentRunLine`. Schema migration: `update_replenishment_schema.py`. Tuesday/Friday ordering cycles with receipt date calculations, pre-receipt demand deduction, cover period forecasting. Warning system with priority ordering: CASE_QTY_MISSING > NEGATIVE_PROJECTED_STOCK > EXPIRY_SOON > HAS_ORDERED_STOCK > HAS_CURRENT_RESERVED > ZERO_ORDER.
