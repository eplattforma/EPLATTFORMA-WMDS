...
- **Core Features**:
    ...
    - **Dropbox Integration**: OAuth2 code flow with offline refresh tokens for stable long-lived Dropbox access. Refresh tokens encrypted at rest using Fernet (AES-CBC, key derived from SESSION_SECRET). Single-active-credential rule enforced on reconnect (old credentials marked `replaced`). Stores credentials in `external_auth_credentials` table (provider, refresh_token encrypted, access_token, expiry, Dropbox email/account_id, status). All sync attempts logged in `external_file_sync_log` with expanded statuses: success, success_no_change, auth_error, download_error, parse_error, config_error, running, skipped_concurrent. Concurrency protection prevents duplicate syncs (15-min lock window). Unchanged-file optimization using Dropbox content_hash comparison. Admin UI at `/admin/integrations/dropbox` (blueprint: `dropbox_integration` in `routes_dropbox.py`) with connection status, file config, sync now/test/disconnect/reconnect buttons, last-attempt/last-success summary cards, sync history table with status filter (All/OK/Auth/DL/Parse), prominent reconnect-required banner on auth_error. Service layer: `services/dropbox_service.py` with automatic token refresh, retry-on-401, 5xx/429 retry with backoff, CSRF-protected POST routes, admin-only access. Parser validates Excel headers, column counts, deduplicates, records row-level metadata (total_rows, skipped, parse_errors, deduped). Scheduled job: `python -m jobs.dropbox_sync` (exit 0 on success/unchanged, exit 1 on failure). Default processor: stock positions from `SerialsStockPositionReport1.xlsx`. Secrets: `DROPBOX_APP_KEY`, `DROPBOX_APP_SECRET`, `DROPBOX_REDIRECT_URI`, `DROPBOX_FILE_PATH`.
