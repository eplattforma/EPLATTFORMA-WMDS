...
- **Core Features**:
    - **Picking System**: Batch reports and Sequential picking order read route/stop from the Routes module (active `route_stop_invoice` links via `services/route_links.py`) — legacy `Invoice.routing` is mostly dead (~7% populated) and only used as a fallback label. Supports individual and batch picking, skip/collect later functionality, real-time updates, and exception handling. Includes a refactored batch picking system with DB-backed queues, and cooler picking capabilities for temperature-sensitive items. **Picker-speed reporting** uses `vw_pick_detail` / `vw_picker_daily` (created idempotently at boot by `migrations/picking_report_views.py`, dev AND prod) built on `item_time_tracking` with MEDIAN seconds per pick — never `pbi_fact_picking`, whose order wall-clock durations include breaks (it stays as-is for order status/shipping/delivery). Report pages: `/reports/picking-performance` (with pick-detail drill-down) and `/reports/idle-dedicated` (`vw_idle_dedicated` — idle shown only for usernames in the `dedicated_pickers` setting). The old "Time Analysis" (`/admin/time_analysis`, `routes_time_analysis.py`) was deleted; "Time Reports" (`/shift/reports`) now sources items/% meeting target from `vw_picker_daily`, computes Items/Hour from `active_pick_hours` (active picking time, never padded shift hours), and shows "Gaps between picks" clamped to the worked window (check-in → last pick) for dedicated pickers only. The EOD auto-close (`check_for_missed_checkouts`) closes shifts at the picker's last real activity (MAX `item_completed`, fallback ActivityLog, fallback EOD, never before check-in) instead of stamping the fixed EOD clock time — no more phantom hours/idle tails. `item_time_tracking.level` was backfilled once (marker setting `picking_level_backfill_done`) and new writes parse it via `parse_location_components` (handles `30-07-C02` → corridor 30, shelf 07, level C, bin 02).
    ...
    - **DW Invoice Dating**: `dw_invoice_header` stores both `invoice_date_utc0` and `invoice_date_local` (PS365 "value date"). All reporting (`pbi_fact_sales`, KPI dashboard) uses `COALESCE(invoice_date_local, invoice_date_utc0)` because PS365's own reports and its header API from/to filter work on the value date, which can differ from utc0 by months. `pbi_fact_sales` also carries a per-invoice HEADER_ADJ reconciliation arm (item_code NULL) so SUM(line_total_excl) always ties out to header net (`total_sub - total_discount`), plus a UNION arm for manually imported credit notes (`dw_credit_note`, import UI under DW menu).
    ...
    - **Scheduler Management**: Every scheduled job is wrapped for tracking, logging, and progress reporting. Includes an admin UI for managing jobs (reschedule, pause, resume, run now) and a log cleanup service. Boot-time registration in `scheduler.setup_scheduler` preserves user-edited triggers persisted in `apscheduler_jobs` (via the `_add_job_smart` helper) — code-default `CronTrigger(...)` values only apply on first registration; subsequent edits via the UI survive worker boots. To force a code default to take effect, delete the row from `apscheduler_jobs` for that job id. **Timezone**: a single `SCHEDULER_TZ='Africa/Cairo'` constant is shared by the live designated-worker scheduler, the `_JobstoreContext` proxy used by non-designated workers, and `reschedule_job`'s `CronTrigger`. UI inputs like "09:38" therefore always mean Cairo wall-clock, even when the request lands on a non-designated worker. A boot-time healer in `setup_scheduler` re-stamps any persisted CronTrigger with a non-Cairo tz (e.g. legacy UTC-bound rows from before this fix) to `Africa/Cairo` while preserving the user's hour/minute/dow.
