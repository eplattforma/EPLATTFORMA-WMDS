...
- **Core Features**:
    ...
    - **Communications Hub**: Unified multi-channel customer communications at `/admin/communications`. Supports 5 channels: MicroSMS (API-sent), Phone SMS, Phone Call, WhatsApp, Viber (last 4 via launch URLs). Features: per-customer compose page with template selection and preview, call outcome modal, live communication history refresh, bulk send from Review Ordering page. Tables: `crm_communication_log` (individual messages with DLR tracking), `crm_communication_batch` (bulk operations). Service layer in `services/communications_service.py` with phone normalization, template rendering, dual-write to legacy `sms_log`. DLR handler updates both `sms_log` and `crm_communication_log`.
