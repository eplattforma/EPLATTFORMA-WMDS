...
- **Core Features**:
    ...
    - **Dropbox Integration**: OAuth2 code flow with offline refresh tokens for stable long-lived Dropbox access. Stores credentials in `external_auth_credentials` table (provider, refresh_token, access_token, expiry, Dropbox email/account_id, status). All sync attempts logged in `external_file_sync_log` (provider, file_path, status, rows_imported, revision, timestamps, errors). Admin UI at `/admin/integrations/dropbox` (blueprint: `dropbox_integration` in `routes_dropbox.py`) with connection status, file config, sync now/test/disconnect/reconnect buttons, sync history table. Service layer: `services/dropbox_service.py` with automatic token refresh, retry-on-401, CSRF-protected POST routes, admin-only access. Scheduled job: `python -m jobs.dropbox_sync`. Default processor: stock positions from `SerialsStockPositionReport1.xlsx`. Secrets: `DROPBOX_APP_KEY`, `DROPBOX_APP_SECRET`, `DROPBOX_REDIRECT_URI`, `DROPBOX_FILE_PATH`.
