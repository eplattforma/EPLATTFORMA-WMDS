...
- **Core Features**:
    ...
    - **Communications Hub**: Unified multi-channel customer communications at `/admin/communications`. Supports 6 channels: MicroSMS (API-sent), Phone SMS, Phone Call, WhatsApp, Viber (last 4 via launch URLs), and OneSignal Push Notifications. Features: per-customer compose page with template selection and preview, call outcome modal, live communication history refresh, bulk send from Review Ordering page, push notification with title/URL fields and real-time subscription verification. **Push Destination**: Target type selector (none/category/product/custom_deeplink) with conditional input fields; routing data sent in OneSignal `data` payload (`target_type`, `category_id`, `product_id`, `deep_link`); custom deep links also set `url`; validated server-side with strict allowlist; logged in `crm_communication_log` columns `push_target_type`, `push_target_id`, `push_deep_link`, `push_data_json` (JSONB). Tables: `crm_communication_log` (individual messages with DLR tracking + push target fields), `crm_communication_batch` (bulk operations), `customer_push_identity` (OneSignal push subscription cache). Service layer in `services/communications_service.py` with phone normalization, template rendering, dual-write to legacy `sms_log`. Push service in `services/onesignal_service.py` with user lookup, subscription verification, push send via OneSignal API, identity caching, and `_build_push_data` helper for payload construction. DLR handler updates both `sms_log` and `crm_communication_log`. Template `allow_onesignal_push` channel flag with badge in template list.
    - **Customer Price Master Import**: Imports `customer_price_master.csv` from FTP (daily at 4:30 AM) containing customer-specific pricing from Magento/BSS. Raw data stored in `crm_customer_price_offer_import` (audit trail), normalized into `crm_customer_price_offer` with customer linking (via `magento_customer_last_login_current` mapping magento_customer_id → customer_code_365) and item linking (SKU → `ps_items_dw.item_code_365`). Calculates discount_amount/discount_percent. CRM dashboard shows pricing chip in detail rows; modal displays summary badges and searchable/sortable offer table. API endpoints: `/crm/customer/<code>/price-offer-summary` and `/crm/customer/<code>/price-offers`. Service: `services/crm_price_offers.py`. Safety: batch quality threshold check before stale row removal.
    - **FTP Login Sync**: Automated every 30 minutes (at :15 and :45) via `services/ftp_login_sync.py`. Downloads `customer_login_logs.csv` from FTP server, imports new login records into `magento_customer_login_log`, refreshes `magento_customer_last_login_current` table.
