...
- **Order readiness:** `services/order_readiness.is_order_ready(invoice_no)` aggregates pick state across normal + cooler queues and returns a single boolean. After the architect fix-up (FIX-02, ASSUMPTION-025) it short-circuits when `summer_cooler_mode_enabled=false`: only normal/NULL queue rows are inspected, the `cooler_boxes` tables are ignored, and behaviour matches the pre-Phase-5 readiness check exactly. Every "ready for dispatch" gate in `routes_routes.py` (the two DISPATCHED-transition gates and the `all_ready_for_dispatch` flag — lines ~278, ~1129, ~1455) now delegates to this helper instead of comparing raw `Invoice.status` (FIX-03, ASSUMPTION-026) — single source of truth.
...
- **Architect fix-up (Task #22, May 3 2026):** Five defects from the architect rejection are closed (see `PHASE_TEST_RESULTS.md` Phase 5 fix-up section): FIX-01 `rebuild_items_from_queue` excludes cooler-zone rows so the normal picker never double-picks cold chain; FIX-02 above; FIX-03 above; FIX-04 `blueprints/cooler_picking.route_picking` scopes `cooler_boxes` by `route_id AND delivery_date` (not date alone) so sibling routes no longer leak boxes onto the picker screen; FIX-05 `update_phase5_cooler_picking_schema.py` is dialect-aware (Postgres `BIGSERIAL`/TS-TZ in production, SQLite `INTEGER PRIMARY KEY AUTOINCREMENT`/`TIMESTAMP` in tests; column adds via `inspect()` reflection — ASSUMPTION-027) so the migration runs cleanly on cold boot under both backends.
- **Tests:** `tests/test_phase5_cooler_picking.py` 40/40 passing (33 original + 7 fix-up regressions in `TestArchitectFixupRegressions`). Phase 4 regression suite still 30/30. Override-pipeline failure is a pre-existing prod-DB data gap, not caused by the fix-up.
...
- **Master flag:** `cockpit_enabled` (default `false`, seeded by `services/settings_defaults.py`). When `false`, every `/cockpit/...` route returns HTTP 404 and the "AM Cockpit" menu entry under Customers is hidden. No existing report is touched.
- **Schema (additive, idempotent — `migrations/cockpit_schema.py`):** `customer_spend_target` (one row per customer with cadence + `status`/proposed/active fields), `customer_spend_target_history` (append-only audit log), and Postgres-only `vw_customer_offer_opportunity` view. Wired into `main.py` after the Phase 5 schema runner; logs `Cockpit: schema ensured` on every boot. Dialect-aware: SQLite skips the view (the cockpit service degrades to `[]` rather than raising).
- **Blueprint (`blueprints/cockpit.py`, prefix `/cockpit`):** routes for the customer search landing page, customer cockpit page (placeholder until Ticket 2 — already shows live target + 4-period achievement), targets admin page, and a target API surface (`get/propose/set/approve/reject/history`). Master-flag check is a `before_request` 404; per-route gating is via `@require_permission` with the new keys.
- **Permissions (registered, intentionally unassigned):** `menu.cockpit`, `customers.use_cockpit`, `customers.propose_target`, `customers.approve_target`, `customers.ask_claude` (listed in `services.permissions.COCKPIT_PERMISSION_KEYS`). Admins still receive them via the `*` wildcard. Claudio assigns to specific users at rollout.
- **Targets workflow:** AM proposes (`status='proposed'`, active target preserved); manager approves (`status='active'`, two history rows: `approved` + `active_snapshot`), rejects (active unchanged, `rejected` history row with reason), or sets directly (`status='active'`, single `set` row). Annual-only payloads auto-populate monthly/quarterly/weekly (annual ÷ 12 / 4 / 52 — ASSUMPTION-035). Inline-edit on the admin page persists via `POST /cockpit/api/<code>/target/set`.
- **Views surface used:** `vw_customer_offer_opportunity` joins `dw_invoice_line` ↔ `dw_invoice_header` for invoice date and `line_total_excl` for revenue, peers grouped by `ps_customers.reporting_group`, ≥ 3 peer threshold (ASSUMPTION-034 documents the schema mapping vs the brief's placeholders).
- **Verified at boot:** `customer_spend_target` and `customer_spend_target_history` tables present, view returns rows on Postgres, propose → approve → set → propose → reject → 10-event history workflow round-trips end to end against a real `ps_customers` row, override-pipeline regression suite still passes.
