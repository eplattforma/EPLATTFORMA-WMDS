...
- **Core Features**:
    ...
    - **DW Cost Enrichment**: Invoice lines in the data warehouse (`dw_invoice_line`) for 2026+ invoices are enriched with cost snapshot fields (`unit_cost_snapshot`, `line_cost_total`, `gross_profit`, `gross_margin_pct`, `cost_source`, `cost_snapshot_at`) sourced from `DwItem.cost_price`. Pre-2026 invoices remain with null cost fields. Operator-triggered invoice sync (`background_sync.py`) automatically triggers a lightweight DW catch-up after successful operational sync. Single-invoice catch-up uses `sync_invoice_to_dw()` (scoped preload by invoice number); date-based catch-up uses `sync_invoices_from_date()` with scoped preload by date range. Both skip MV refresh (`refresh_mv=False`). Scoped preload helpers (`_load_existing_invoice_headers_map`, `_load_existing_invoice_line_keys`) avoid full-table scans. Nightly DW cron retains full MV refresh behavior. Background sync status lifecycle: `completed_at` set only at true process end, `running=False` in `finally`, operational failure early-exits before DW catch-up.
