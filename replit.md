...
- **Invoice Import (PS365)**: Optimized single-pass synchronization logic with data normalization (_norm_code, _norm_barcode), batch lookups for shelf locations and barcodes, and automated invoice total recalculation. Prevents duplicates and ensures high data integrity for barcodes. Supports async date imports via background job system with isolated database sessions.
- **Background Job System**: Async job execution for long-running imports (`sync_jobs.py`). Uses thread-isolated SQLAlchemy sessions to prevent session corruption. API endpoints: `POST /api/powersoft/sync/invoices/async` (start job), `GET /api/powersoft/jobs/{job_id}` (poll status), `GET /api/powersoft/jobs` (list recent jobs). Job status tracked in `sync_jobs` table with progress, results, and error handling.
