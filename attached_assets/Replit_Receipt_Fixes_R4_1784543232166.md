...
## Part 3 — Show voided receipts on route reconciliation

`routes_routes.py` (~line 568) excludes `status = 'VOIDED'` receipts from the route reconciliation screen entirely. Correct for totals, wrong for display: reconciliation is where the driver hands in cash **and cancelled customer copies**, so the reconciler must see the voids to collect the paper.

1. Include voided receipts in the query; render them as a distinct **VOIDED** row (struck-through amount, red badge) showing: void reason, voided by/at, customer copies expected (`print_count`) vs recovered (`slips_recovered`), PS365 cancellation ref, and a link to the replacement receipt.
2. Keep them **out of all cash/variance totals** (current math unchanged).
3. If `slips_recovered` is null or `< print_count`, flag the row: "copies outstanding — collect from driver."
...
11. Route reconciliation shows a voided receipt as a struck-through VOIDED row with reason and copies expected/recovered; cash totals unchanged; outstanding copies flagged.
