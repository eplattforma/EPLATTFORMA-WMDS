# EP SmartGrowth CRM — Fix Value 4w and GP% Using Net Sales (Excl VAT)

## Root cause

The `sales_sq` subquery in `routes_crm_dashboard.py` uses `DwInvoiceHeader.total_grand`
(gross total **including VAT**) for all value calculations. The correct field is
`DwInvoiceHeader.total_sub` (subtotal **excluding VAT**), which matches the Net Value
column in the system's power report.
...
Only `routes_crm_dashboard.py` (Changes 1 & 2) and wherever `gp_sq` was added
(Change 3 — likely also in `routes_crm_dashboard.py`). No model, template, or
