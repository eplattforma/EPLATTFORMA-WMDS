...
class ActivityService:
    ...
    def check_in(self, username: str, coordinates: Optional[str] = None) -> OpenShift:
        ...
        with self._cursor(commit=True) as cur:
            ...
            if existing:
                ...
                cur.execute(
                    """INSERT INTO shifts
                       ...
                       VALUES (%s, now()::timestamp, %s, 'active') RETURNING id""",
            ...
            # segment must start at check_in_time, not now(). ensure_open handles
    ...
    def check_out(self, shift_id: int, coordinates: Optional[str] = None) -> dict:
        ...
        with self._cursor(commit=True) as cur:
            cur.execute(
                "SELECT picker_shift_close(%s, now()::timestamp, 'check_out')",
                (shift_id,),
            )
            if coordinates:
                cur.execute(
        ...
        try:
            row = self._exec(
                """SELECT picker_transition(
                       %s, %s::picker_state, now()::timestamp, %s, %s, %s::uuid, %s
                   ) AS id""",
    ...
    def heartbeat(self, shift_id: int) -> None:
        ...
        break."""
        self._exec(
            "UPDATE shifts SET last_heartbeat_at = now()::timestamp WHERE id = %s",
            (shift_id,),
        )
