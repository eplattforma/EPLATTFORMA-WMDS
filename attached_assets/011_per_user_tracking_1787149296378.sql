...
BEGIN
  IF OLD.track_activity AND NOT NEW.track_activity THEN
    ...
    LOOP
      PERFORM picker_shift_close(r.id, now()::timestamp, 'admin');
    END LOOP;
  END IF;
  RETURN NEW;
END $$ LANGUAGE plpgsql;
...
BEGIN
  ...
  UPDATE shifts
     SET last_heartbeat_at = COALESCE(last_heartbeat_at, now()::timestamp)
   WHERE id = p_shift_id;
...
BEGIN
  UPDATE picker_segment
     SET resolution = 'declined', resolved_by = p_by, resolved_at = now()
...
BEGIN
  IF p_state IS NULL THEN
    UPDATE picker_segment
       SET resolution='written_off', resolved_by=p_by, resolved_at=now(), resolution_note=p_note
     WHERE id = p_segment_id;
  ELSE
    UPDATE picker_segment
       SET state=p_state, reclassified_from=state, reclassified_at=now(), reclassified_by=p_by,
           resolution='supervisor_set', resolved_by=p_by, resolved_at=now(), resolution_note=p_note
     WHERE id = p_segment_id;
  END IF;
END $$ LANGUAGE plpgsql;
...
BEGIN
  UPDATE picker_segment
     SET resolution='written_off', resolved_by='system',
         resolved_at=now(), resolution_note='auto write-off after '||v_days||' days'
   WHERE resolution IN ('pending','declined')
     AND ended_at < now() - make_interval(days => v_days);
...
SELECT g.id AS segment_id, g.shift_id, g.picker_username,
       ...
       g.resolution,
       (now()::timestamp - g.ended_at) AS age
