...
The picker now sees one card per box instead of a flat table:

```
┌─────────────────────────────────────────────────────────────┐
│ 📦 Box #1  Large  Stops 13→8        0/12 picked  [Manifest] │  ← blue header
├─────────────────────────────────────────────────────────────┤
│ Stop │ Customer    │ Item   │ Qty │ Status  │ Action         │
│  13  │ Coffee Shop │ CHO-14 │  1  │ Pending │ Pick→Box #1   │
│  10  │ Petrolina   │ CHO-66 │  1  │ Pending │ Pick→Box #1   │
│   8  │ Annie Belle │ CHO-25 │  2  │ Pending │ Pick→Box #1   │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────┐
│ 📦 Box #2  Medium  Stops 5→1        0/4 picked  [Manifest]  │
├─────────────────────────────────────────────────────────────┤
│   5  │ GMK Kiosk   │ CHO-27 │  1  │ Pending │ Pick→Box #2  │
│   3  │ CoffeeSense │ CHO-11 │  1  │ Pending │ Pick→Box #2  │
│   1  │ EKO Station │ CHO-14 │  1  │ Pending │ Pick→Box #2  │
└─────────────────────────────────────────────────────────────┘
```

- Header turns **green** and **"Close Box"** button appears automatically when all items in that box are picked
- Items start from highest stop number (last delivery = deepest in truck = picked first)
- Items with no box assignment show in a separate yellow warning card at the bottom
- Each box header has a **Print Manifest** button
