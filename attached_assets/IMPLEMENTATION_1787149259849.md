...
| `sql/011_per_user_tracking.sql` | Per-user gate, `awaiting_order`, resolution machine, `ensure_open`, accounting rebuild | ran clean |
...
```bash
pip install psycopg2-binary        # or psycopg[pool] for psycopg 3
```

```bash
# 1. BRANCH YOUR NEON DATABASE FIRST — this is DDL against production.
#    Neon console -> Branches -> Create branch. Run steps 2-3 there, verify, promote.

# 2. Migrations, in order
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f sql/010_picker_timeline_postgres.sql
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f sql/011_per_user_tracking.sql
...
## 8. Four defects this testing caught
