...
BEGIN
  ...
  IF FOUND THEN
    UPDATE picker_segment
       SET ended_at = GREATEST(p_at, v_open.started_at + interval '1 second'),
           close_reason = p_close_reason
     WHERE id = v_open.id;
  END IF;

  UPDATE shifts
     SET check_out_time = COALESCE(check_out_time, p_at),
         total_duration_minutes = COALESCE(
           total_duration_minutes,
           (EXTRACT(EPOCH FROM (p_at - check_in_time)) / 60)::int),
