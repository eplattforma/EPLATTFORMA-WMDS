        ...
        try:
            ...
            with db.engine.connect() as conn:
                for col_name, col_def in [
                    ...
                    ('target_weeks_updated_at', 'TIMESTAMP NULL'),
                    ('target_weeks_updated_by', 'VARCHAR(100) NULL'),
                ]:
            ...
            logging.warning(f"Could not add forecast profile columns: {str(e)}")

        try:
            from sqlalchemy import text as _sa_text2
            with db.engine.connect() as conn:
                r = conn.execute(_sa_text2(
                    "SELECT 1 FROM information_schema.tables WHERE table_name='sku_ordering_snapshot'"
                ))
                if not r.fetchone():
                    conn.execute(_sa_text2("""
                        CREATE TABLE sku_ordering_snapshot (
                    ...
                    """))
                    conn.commit()
                    logging.info("Created sku_ordering_snapshot table")
        except Exception as e:
            logging.warning(f"Could not create sku_ordering_snapshot table: {str(e)}")
