...
_BLOCKERS = {
    "normal_batch":     "Normal picking pending",
    "cooler_batch":     "Cooler picking pending",
    "cooler_unboxed":   "Cooler items unboxed",
    "open_boxes":       "Cooler boxes open",
    "exceptions":       "Picking exceptions unresolved",
    "pending_queue":    "Cooler items pending pick",
}
...
def check_route_warehouse_ready(route_id: int) -> Tuple[bool, List[str]]:
    ...
    try:
        # 1. All normal (non-cooler) batch sessions must be Completed/Cancelled
        pending_batches = db.session.execute(
            db.text(
        ...
        if pending_batches > 0:
            blockers.append(
                f"{_BLOCKERS['normal_batch']} ({pending_batches} batch(es))"
            )

        # 2. All cooler batch sessions must be Completed/Cancelled
        ...
        if pending_cooler > 0:
            blockers.append(
                f"{_BLOCKERS['cooler_batch']} ({pending_cooler} session(s))"
            )

        # 3 & 6. No picked cooler items that are not yet assigned to a box
        ...
        if unboxed > 0:
            blockers.append(
                f"{_BLOCKERS['cooler_unboxed']} ({unboxed} item(s) picked but not in a box)"
            )

        # 4. All cooler boxes for the route must be closed (not 'open')
        ...
        if open_boxes > 0:
            blockers.append(
                f"{_BLOCKERS['open_boxes']} ({open_boxes} box(es) still open)"
            )

        # 5. No unresolved picking exceptions for invoices on this route
        unresolved_exc = db.session.execute(
            db.text(
                "SELECT COUNT(*) "
        ...
        if unresolved_exc > 0:
            blockers.append(
                f"{_BLOCKERS['exceptions']} ({unresolved_exc})"
            )

        # 7. No pending cooler queue items for this route
        pending_queue = db.session.execute(
            db.text(
                "SELECT COUNT(*) "
                "FROM batch_pick_queue bpq "
                "JOIN batch_picking_sessions bps ON bps.id = bpq.batch_session_id "
        ...
        if pending_queue > 0:
            blockers.append(
                f"{_BLOCKERS['pending_queue']} ({pending_queue} item(s))"
            )
