  ...
  FOR r IN
    SELECT s.id, COALESCE(s.last_heartbeat_at, s.check_in_time) AS cut
     ...
     WHERE s.check_out_time IS NULL
       AND (now() AT TIME ZONE 'utc') - s.check_in_time > make_interval(hours => v_max_hours)
  LOOP
  ...
  FOR r IN
    SELECT s.id, s.last_heartbeat_at AS cut
     ...
     WHERE s.check_out_time IS NULL
       ...
       AND (now() AT TIME ZONE 'utc') - s.last_heartbeat_at > make_interval(secs => v_hb_timeout)
       AND g.state <> 'offline'
...
WITH bounds AS (
  SELECT s.id                AS shift_id,
         s.picker_username,
         s.check_in_time,
         COALESCE(s.check_out_time, (now() AT TIME ZONE 'utc')) AS effective_end,
         s.status
    FROM shifts s
),
cover AS (
...
-- ----------------------------------------------------------------------------
-- Ordered DROPs, not CREATE OR REPLACE: migration 012 widens this view with
-- awaiting_order, and a replace cannot narrow it back on re-run.
-- vw_accounting_reconciliation (created in 012) selects from it, so it must
-- be dropped first. Explicit order rather than CASCADE.
DROP VIEW IF EXISTS vw_accounting_reconciliation;
...
--      INSERT INTO picker_segment (shift_id, picker_username, state, started_at, opened_by)
--      VALUES (<shift_id>, 'Arslan', 'break', (now() AT TIME ZONE 'utc'), 'picker');
--    ROLLBACK;

-- C. Idempotency: same action_id twice returns the same segment id, creates one row.
--    SELECT picker_transition(<shift_id>,'break',(now() AT TIME ZONE 'utc'),'picker','declared',
